...
/**
 * Sends [4-byte Length Header] + [Payload]
 */
...
/**
 * Receives [4-byte Length Header] + [Payload]
 * Returns: Number of payload bytes received, or < 0 on error
 */
int recv_frame(int sock, uint8_t *buffer, size_t max_buffer_len) {
    uint32_t net_len = 0;
    int received = 0;
    ...
    if (payload_len > max_buffer_len) {
        ESP_LOGE(TAG, "Incoming packet too large: %lu", payload_len);
        return -2; 
    }
